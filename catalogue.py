...
def load(frame_left, frame_right, conn):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT rowid, ref_fab, marque, designation, commentaires, ref_ugap FROM catalogue_fab")
    ...
    for i, row in enumerate(rows):
        _, ref_fab, marque, desig, _, _ = row
        listbox.insert(tk.END, f"{ref_fab} - {desig}")
        data_by_index[i] = row
    ...
    def display_details(row):
        ...
        champs = ["rowid","ref_fab","marque","designation","commentaires","ref_ugap"]
    ...
    def show_form(data):
        for w in details_frame.winfo_children(): w.destroy()
        champs = ["ref_fab","marque","designation","commentaires","ref_ugap"]
        entries = {}
        ...
        def valider():
            ...
            if data is None:
                cur.execute(
                    "INSERT INTO catalogue_fab (ref_fab, marque, designation, commentaires, ref_ugap) VALUES (?,?,?,?,?)",
                    vals)
            else:
                key = data[0]
                cur.execute(
                    "UPDATE catalogue_fab SET ref_fab=?, marque=?, designation=?, commentaires=?, ref_ugap=? WHERE rowid=?",
                    vals + [key])
            conn.commit(); refresh()
    ...
    def delete_selected():
        ...
        cur = conn.cursor()
        cur.execute("DELETE FROM catalogue_fab WHERE rowid=?", (key,))
        conn.commit(); refresh()
