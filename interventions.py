...
def load(frame_left, frame_right, conn):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT rowid, date_debut, date_fin, duree, qui, sur_quoi, description, pieces, type, pj, devis, reception, etat, planifie, pjint FROM interventions")
    ...
    def display_details(row):
        # Affiche détails et boutons
        for w in details_frame.winfo_children(): w.destroy()
        champs = ["rowid","date_debut","date_fin","duree","qui","sur_quoi","description","pieces","type","pj","devis","reception","etat","planifie","pjint"]
    ...
    def show_form(data):
        ...
        for w in details_frame.winfo_children(): w.destroy()
        champs = ["date_debut","date_fin","duree","qui","sur_quoi","description","pieces","type","pj","devis","reception","etat","planifie","pjint"]
        entries = {}
        ...
        def valider():
            ...
            if data is None:
                cur.execute(
                    "INSERT INTO interventions (date_debut,date_fin,duree,qui,sur_quoi,description,pieces,type,pj,devis,reception,etat,planifie,pjint) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    vals)
            else:
                key = data[0]
                cur.execute(
                    "UPDATE interventions SET date_debut=?,date_fin=?,duree=?,qui=?,sur_quoi=?,description=?,pieces=?,type=?,pj=?,devis=?,reception=?,etat=?,planifie=?,pjint=? WHERE rowid=?",
                    vals + [key])
            conn.commit(); refresh()
    ...
    def delete_selected():
        ...
        cur = conn.cursor()
        cur.execute("DELETE FROM interventions WHERE rowid=?", (key,))
        conn.commit(); refresh()
